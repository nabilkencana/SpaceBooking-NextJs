...
export function SmoothScroll() {
  useEffect(() => {
    ...
    // 3. Tangani navigasi anchor link internal (misal: #options atau /#options)
    const handleAnchorClick = (e: MouseEvent) => {
      ...
      if (rawHref.startsWith("#") && rawHref.length > 1) {
        hash = rawHref;
      } else if (rawHref.startsWith("/#") && rawHref.length > 2) {
        if (typeof window !== "undefined" && window.location.pathname === "/") {
          hash = rawHref.substring(1);
        }
      }
    ...
    // 4. Tangani anchor hash saat awal masuk halaman (misal direct URL atau redirect dari halaman lain)

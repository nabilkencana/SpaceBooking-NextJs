...
/* Giant Background Watermark Container */
.footer-giant-wrapper {
  transform-style: preserve-3d;
  will-change: transform, opacity;
  mask-image: linear-gradient(to bottom, black 50%, rgba(0, 0, 0, 0.9) 75%, rgba(0, 0, 0, 0.4) 100%);
  -webkit-mask-image: linear-gradient(to bottom, black 50%, rgba(0, 0, 0, 0.9) 75%, rgba(0, 0, 0, 0.4) 100%);
}

/* Precision Clean Vector Outline (Eliminates all variable font contour & overlap glitches) */
.footer-giant-outline {
  ...
  color: #000;
  filter: url(#urspace-footer-clean-outline);
  user-select: none;
  pointer-events: none;
}
      ...
      */}
      ...
      >
        ...
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-[#FAFAFA] text-brand-dark cinematic-footer-wrapper border-t border-[#E5E7EB]">
          ...
          {/* SVG Vector Filter for 100% Mathematically Flawless Outline (Zero Glyph Overlaps) */}
          <svg width="0" height="0" className="absolute pointer-events-none opacity-0" aria-hidden="true">
            <defs>
              <filter id="urspace-footer-clean-outline" x="-20%" y="-20%" width="140%" height="140%">
                {/* 1. Extract pure outer perimeter via morphology subtraction */}
                <feMorphology in="SourceAlpha" result="DILATED" operator="dilate" radius="2" />
                <feComposite in="DILATED" in2="SourceAlpha" result="OUTLINE" operator="out" />

                {/* 2. Soft ambient violet glow */}
                <feGaussianBlur in="OUTLINE" stdDeviation="8" result="GLOW" />
                <feFlood floodColor="#5E43F3" floodOpacity="0.30" result="GLOW_COLOR" />
                <feComposite in="GLOW_COLOR" in2="GLOW" operator="in" result="COLORED_GLOW" />

                {/* 3. Crisp architectural slate outline stroke */}
                <feFlood floodColor="#111827" floodOpacity="0.22" result="STROKE_COLOR" />
                <feComposite in="STROKE_COLOR" in2="OUTLINE" operator="in" result="COLORED_STROKE" />

                {/* 4. Layer crisp stroke over ambient glow */}
                <feMerge>
                  <feMergeNode in="COLORED_GLOW" />
          ...
          {/* Giant background text (Bespoke Clean Architectural Watermark) */}
          <div
            ref={giantTextRef}
            className="footer-giant-wrapper absolute bottom-[-3vh] sm:bottom-[-4vh] left-1/2 -translate-x-1/2 whitespace-nowrap z-0 pointer-events-none select-none"
          >
            <div className="footer-giant-outline" aria-hidden="true">
              {giantText}
            </div>
          </div>
